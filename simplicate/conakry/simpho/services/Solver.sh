...
VER=1.0
LAUNCH=SolverBLImplServer_Corba
LOG=log4j.properties
CWDIR=`pwd`
LPATH=com/ocs/simplicate/idl
CMD=-s
...
if [ x"$SOLVERPID" != x"0" ]; then
    ...
    if [ x"$RESULT" == x"0" ]; then
        ...
        echo "Message: Successfully killed ${LAUNCH} ${SOLVERPID}."
        ...
        echo "Warning: Process ID ${SOLVERPID} was not found."
    fi
...
# set additional variables (WEB-INF) directory
SBL=${SIMPLICATE}
if [ ! -d ${SBL} ]; then
...
java -Dlog4j.configuration=log4j.SolverBL.properties ${CATALINA_OPTS} ${LPATH}/${LAUNCH} &
if [ "$?" -ne "0" ]; then
...
echo "Message: ${LAUNCH} started. PID=${SOLVERPID}"
export CLASSPATH=$DUMMY
