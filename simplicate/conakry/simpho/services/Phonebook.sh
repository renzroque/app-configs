...
VER=1.0
LAUNCH=PhonebookServer
CWDIR=`pwd`
LPATH=com/ocs/simplicate/service/phonebook
...
java -Dorg.omg.CORBA.ORBSingletonClass=org.openorb.CORBA.ORBSingleton -Dorg.omg.CORBA.ORBClass=org.openorb.CORBA.ORB -Dlog4j.configuration=log4j.PhonebookBL.properties  ${LPATH}/${LAUNCH} ${CATALINA_OPTS} -ORBPort=${PORT} &
if [ "$?" -ne "0" ]; then
