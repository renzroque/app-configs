...
CWDIR=`pwd`
LPATH=com/ocs/simplicate/service/promotion
...
if [ x"$CAMPAIGNPID" != x"0" ]; then
    ...
    if [ x"$RESULT" == x"0" ]; then
        ...
        echo "Message: Successfully killed ${LAUNCH} ${CAMPAIGNPID}."
        ...
        echo "Warning: Process ID ${CAMPAIGNPID} was not found."
    fi
...
PID=`ps -ef | grep "$LAUNCH" | grep "$LOGNAME"  | grep "$PORT"| awk '{print $2} '`
...
java -Dorg.omg.CORBA.ORBSingletonClass=org.openorb.CORBA.ORBSingleton -Dorg.omg.CORBA.ORBClass=org.openorb.CORBA.ORB -Dlog4j.configuration=log4j.CampaignBL.properties ${LPATH}/${LAUNCH} ${CATALINA_OPTS} -ORBPort=${PORT} &
if [ "$?" -ne "0" ]; then
...
echo "Message: ${LAUNCH} started. PID=${CAMPAIGNPID}"
export CLASSPATH=$DUMMY
