...
# SimSessionLog launch script
# created by: Michael Remus G. Gaerlan
# creation date: 12-16-2004

# variable initialization
VER=1.0
LAUNCH=SimSessionLogServer
CWDIR=`pwd`
LPATH=com/ocs/simplicate/service/simsession/server
...
# perform stop
if [ x"$SESSIONLOGPID" != x"0" ]; then

    kill -9 $SESSIONLOGPID
    RESULT=$?

    if [ x"$RESULT" == x"0" ]; then

        echo "Message: Successfully killed ${LAUNCH} ${SESSIONLOGPID}."
        ...
        echo "Warning: Process ID ${SESSIONLOGPID} was not found."
    fi
...
java -Dorg.omg.CORBA.ORBSingletonClass=org.openorb.CORBA.ORBSingleton -Dorg.omg.CORBA.ORBClass=org.openorb.CORBA.ORB -Dlog4j.configuration=log4j.SimSessionBL.properties ${LPATH}/${LAUNCH} ${CATALINA_OPTS} -ORBPort=${PORT} &
#java -server -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,address=8001,server=y,suspend=n ${LPATH}/${LAUNCH} -ORBPort=${PORT} &
if [ "$?" -ne "0" ]; then
...
export SESSIONLOGPID=$!

echo "Message: ${LAUNCH} started. PID=${SESSIONLOGPID}"
export CLASSPATH=$DUMMY
