...
LAUNCH=ImeiServer
CWDIR=`pwd`
LPATH=com/ocs/simplicate/service/imei
...
if [ x"$IMEIPID" != x"0" ]; then
    ...
    if [ x"$RESULT" == x"0" ]; then
        ...
        echo "Message: Successfully killed ${LAUNCH} ${IMEIPID}."
        ...
        echo "Warning: Process ID ${IMEIPID} was not found."
    fi
...
java -Dorg.omg.CORBA.ORBSingletonClass=org.openorb.CORBA.ORBSingleton -Dorg.omg.CORBA.ORBClass=org.openorb.CORBA.ORB -Dlog4j.configuration=log4j.ImeiBL.properties ${LPATH}/${LAUNCH} ${CATALINA_OPTS} -ORBPort=${PORT} &
if [ "$?" -ne "0" ]; then
...
echo "Message: ${LAUNCH} started. PID=${IMEIPID}"
export CLASSPATH=$DUMMY
